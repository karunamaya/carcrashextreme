...
namespace PG
{
    ...
    public abstract class CreateVehicleWindow :EditorWindow
    {
        ...
        void SetGlassDOParams (GlassDO glassDO, MeshRenderer renderer)
        {
            ...
            glassDO.DestroyEventRef = EditorHelperSettings.GlassDO.DestroyEventRef;
        ...
        void SetLightObjectParams (LightObject lightObject, MeshRenderer renderer)
        {
            ...
            lightObject.DestroyEventRef = EditorHelperSettings.LightObject.DestroyEventRef;
        ...
        protected virtual void OnGUIOther ()
        {
            ...
            if (GUILayout.Button ("Select transform for 1st person camera"))
            {
                if (SelectedController.FirstPersonCameraPos == null)
                {
                    SelectedController.FirstPersonCameraPos = new GameObject ("FirstPerson_CameraPos").transform;
                    SelectedController.FirstPersonCameraPos.SetParent (SelectedController.transform);
                    SelectedController.FirstPersonCameraPos.localPosition = Vector3.zero;
                    SelectedController.FirstPersonCameraPos.localRotation = Quaternion.identity;
                    EditorUtility.SetDirty (SelectedController);
                }
                Selection.activeObject = SelectedController.FirstPersonCameraPos;
                EditorGUIUtility.PingObject (Selection.activeObject);
            }
